...
class DeepseekV2MoE(nn.Module):
    ...
    def __init__(
        ...
        self.routed_scaling_factor = config.routed_scaling_factor
        if self.tp_size > config.n_routed_experts + config.audio_num_experts:
            raise ValueError(
                f"Tensor parallel size {self.tp_size} is greater than "
                f"the number of experts {config.n_routed_experts + config.audio_num_experts}.")

        if config.hidden_act != "silu":
            raise ValueError(f"Unsupported activation: {config.hidden_act}. "
                             "Only silu is supported for now.")

        self.experts = FusedMoE(num_experts=config.audio_num_experts + config.n_routed_experts,
        ...
        self.gate.audio_router_weight = nn.Parameter(
            torch.empty((config.audio_num_experts, config.hidden_size))
        )
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        router_logits_audio = F.linear(
            hidden_states, self.gate.audio_router_weight.to(hidden_states.device), None
        )
        router_logits = torch.concat([router_logits, router_logits_audio], dim=-1)
...
class DeepseekV2ForCausalLM(nn.Module, SupportsPP):
    ...
    def __init__(
        ...
        self.model = DeepseekV2Model(config,
                                     ...
                                     quant_config,
                                     prefix="model")
        ### LUCY self definition begin
        self.lm_head_size = self.config.text_vocab_size + self.config.audio_special_tokens
    ...
    def load_weights(self, weights: Iterable[Tuple[str, torch.Tensor]]):
        ...
        expert_params_mapping = FusedMoE.make_expert_params_mapping(
            ...
            num_experts=self.config.n_routed_experts + self.config.audio_num_experts)
