...
class Qwen2Model(nn.Module):
    ...
    def forward(
        ...
        PAD_A = 4097 #self.config.additional_tokens["PAD_A"]
        ANS_A = 4099#self.config.additional_tokens["ANS_A"]
        PAD_T = 151937
        BOT = 151938 #self.config.additional_tokens["BOT"]
        EOT = 151936 #self.config.additional_tokens["EOT"]
        task = "TQA"
        TASK_TOK = 151941 #self.config.additional_tokens[task]
        audio_num_codebook = 7 #self.config.mm_audio_num_codebook
        text_vocab_size_padded = 152000 #151936 #self.config.text_vocab_size
        # assume batch size is 1
        device = input_ids.device
        input_ids = input_ids.unsqueeze(0)
        #import pdb; pdb.set_trace()
        print("input_ids1=", input_ids, self.first)
        if None in kv_caches:
            ...
            #import pdb; pdb.set_trace()
            if codec_by_layer_padded.shape[1] > text.shape[0]:
                ...
                attn_metadata.num_prefill_tokens += 3
                attn_metadata._cached_prefill_metadata.num_prefill_tokens += 3
            else:
                text_padded = text.unsqueeze(0)
            ...
            #import pdb; pdb.set_trace()
            # for i in range(positions_len, positions_len + 3):
            #     positions = torch.cat((positions, torch.tensor([(positions[positions_len-1] + 1) % 8], device=positions.device)), dim=0)
            self.first -= 1
        else:
            text = input_ids[0]
            text_leng = len(text)
        ...
        print(f"input_ids2={input_ids}")
        #print(f"input_ids2={input_ids}, inputs_embeds={inputs_embeds}")
        #import pdb; pdb.set_trace()
        ...
        #print(f"hidden_states1={hidden_states}")
        ...
        #print(f"hidden_states2={hidden_states}")
        ...
        #print(f"hidden_states3={hidden_states}")
...
class Qwen2ForCausalLM(nn.Module, SupportsLoRA):
    ...
    def __init__(
        ...
        if config.tie_word_embeddings:
            ...
            self.lm_head = ParallelLMHead(self.lm_head_size,
                                          config.hidden_size,
                                          quant_config=quant_config)

        self.logits_processor = LogitsProcessor(self.lm_head_size)
        ...
        for name, loaded_weight in weights:
            ...
            for (param_name, weight_name, shard_id) in stacked_params_mapping:
                ...
                if name == "lm_head.weight":
                    param = torch.zeros_like(loaded_weight, device=param.device, dtype=param.dtype)
                    weight_loader(param, loaded_weight)
                    params_dict[name].data = param
                else:
                    try:
                        weight_loader(param, loaded_weight)
                    except:
