...
DATA_JSONS="$COMMON_DIR/train.json $WEBQA_DIR/train.json"
EVAL_DATA_JSONS="$COMMON_DIR/eval.json $WEBQA_DIR/eval.json"

#TASKS="AQACONVA RQACONVA RQACONV RQACONV RQACONV RQACONVA RQACONV RQACONVA"
TASKS="AQACONVA AQACONV"
...
if [[ -z $DISTRIBUTED_ARGS ]]; then
	...
	#LAUNCH_CMD="deepspeed --include localhost:7 $TRAINING_SCRPT"
else
